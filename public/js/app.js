...
// })
  ...
  $('#searchResults').on('click', '.clickPic', function(){
    ...
    url: 'https://api-public.guidebox.com/v1.43/US/rKwGrhy6qjpZS1rJFNnX0vpdWMRHMJCo/movies/' + id
      ...
      $('.addMovie').remove()
      $('.sources').remove()
      $(`#${data.id}div`).append(`<p class='overview'>${data.overview}</p><p class='overview'>${data.rating} | ${data.release_date}</p><button class='addMovie' onclick="Materialize.toast('Added Movie to Not Watched List', 4000)">Add Movie</button>`)
