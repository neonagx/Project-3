  ...
  $.ajax({
    ...
    if(results.length >= 5) {
      ...
      shortArr.forEach(function(movie){
        console.log(movie)
        $('#searchResults').append(`<div class='searchResponses'><li id='${movie.id}'><a href="">${movie.title}</a></li><img class='clickPic' src="${movie.poster_120x171}">
        <p> ${movie.rating} | ${ movie.release_date} </p></div>`)
      })
    } else {
      results.forEach(function(movie){
        console.log(movie)
        $('#searchResults').append(`<div class='searchResponses'>
        <li id='${movie.id}'><a>${movie.title}</a></li>
        <img class='clickPic' src="${movie.poster_120x171}">
